...
//
// ⚠️ IMax/IType **不在本接口上**：它们只被 overflow 用到，而 overflow 的三个调用点
// (Collection/Document/Values 的 Parse)传的都是具体类型。放在接口上只会逼着
// 不进 IType 体系的句柄(Mount)写两个恒空桩。要那两样的地方声明 overflowHandle，
// 见 funcs.go。
