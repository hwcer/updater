...
type Middleware interface {
	Emit(s *Store, t EventType) (next bool)
}

// 全局中间件，所有 Store 实例共享，每次 emit 都触发，永不移除
...
func (m Middlewares) LoadOrCreate(s *Store, name string, creator func(*Store) Middleware) Middleware {
	if v := m[name]; v != nil {
		return v
	}
	v := creator(s)
...
func (m Middlewares) emit(s *Store, t EventType) {
